...
4.  **Actionable Response**: Gemini returns a JSON object. The backend translates the text (if necessary) via **Google Cloud Translation**, caches the result, and sends it back.
...
*   **Google Cloud Translation API v2**: Used on the backend to dynamically translate the English LLM outputs into 8 Indian regional languages.
...
*   **Google Identity Services (OAuth) & Firestore**: Allows users to seamlessly log in. We only store 3 fields in Firestore (state, language, progress) so users can resume their journey across devices.
*   **Google Calendar API**: Integrated to let users add one-click polling day reminders.
...
7.  The free tier limits (1000 RPD for Gemini, 500K chars for Translate, 1M chars for TTS) are sufficient for demonstration purposes.
